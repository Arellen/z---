...
def process_3782data(id, erp_code, year, month, folder_path, file_path, sheet_name):
    ...
    # 从日期列中提取年份和月份
    df['year'] = df.iloc[:, month_column_index].apply(lambda x: x.year if pd.notnull(x) else None)
    df['month'] = df.iloc[:, month_column_index].apply(lambda x: x.month if pd.notnull(x) else None)
