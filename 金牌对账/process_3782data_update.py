...
def process_3782data(id, erp_code, year, month, folder_path, file_path, sheet_name):
    ...
    # 读取Excel文件中的特定工作表
    df = pd.read_excel(file_path, sheet_name=sheet_name, parse_dates=[month_column_index])
    ...
    # 从日期列中提取年份和月份
    df['year'] = df.iloc[:, month_column_index].dt.year.fillna(-1).astype(int)
    df['month'] = df.iloc[:, month_column_index].dt.month.fillna(-1).astype(int)
    ...
    if round(rounded_values + rounded_prepayment_sum + rounded_deduction_sum, 2) == rounded_balance_value and rounded_deduction_sum == rounded_cost_items_sum:
        ...
        print(f"id: {id}"f"---{previous_month}月份余额+: {rounded_values}+"f"当月收入+: {rounded_prepayment_sum}")